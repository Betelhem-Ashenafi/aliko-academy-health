...
export default function AdminEnterpriseLeadsPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold font-academy">Enterprise Leads</h1>
      ...
      <Dialog open={!!selectedLead} onOpenChange={() => setSelectedLead(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>Lead Details — {selectedLead?.organization_name}</DialogTitle></DialogHeader>
          {selectedLead && (
            <div className="space-y-4">
              ...
              {selectedLead.message && (
                <div><span className="text-muted-foreground font-medium text-sm">Message / Details:</span>
                  <pre className="text-sm whitespace-pre-wrap bg-muted/50 rounded-lg p-3 border mt-1">{selectedLead.message}</pre>
                </div>
              )}
              <div><span className="text-muted-foreground font-medium text-sm">Update Status:</span>
                <Select value={selectedLead.status} onValueChange={(v) => { updateStatus(selectedLead.id, v); setSelectedLead({ ...selectedLead, status: v as any }); }}>
                  ...
                  <SelectContent>{["new", "contacted", "proposal_sent", "won", "lost"].map((s) => (<SelectItem key={s} value={s}>{s.replace(/_/g, " ")}</SelectItem>))}</SelectContent>
                </Select>
              </div>
            </div>
          )}
