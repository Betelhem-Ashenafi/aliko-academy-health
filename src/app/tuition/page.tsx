            ...
            <Card className="overflow-hidden">
              ...
              <div className="block md:hidden p-4 space-y-4">
                {programs.map((program) => {
                  ...
                  const regFee = hasPricing && 'registrationFee' in program ? (program as any).registrationFee : null;
                  const totalCost = hasPricing && 'totalCost' in program ? (program as any).totalCost : null;

                  return (
                    <div key={program.id} className="border border-border rounded-xl p-4 space-y-2 bg-background">
                      <Link href={`/programs/${program.id}`} className="font-semibold hover:text-primary transition-colors block">
                        {program.name}
                      </Link>
                      <div className="text-sm text-muted-foreground">{program.duration} • {program.hours.total} hours</div>
                      <div className="flex items-center justify-between pt-2 border-t border-border">
                        <div className="text-xs text-muted-foreground">
                          <p>Registration: {regFee !== null ? `$${regFee.toLocaleString()}` : 'TBD'}</p>
                          <p>Tuition: {hasPricing ? `$${program.tuition.toLocaleString()}` : 'TBD'}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-xs text-muted-foreground">Total</p>
                          <p className="font-bold text-primary text-lg">
                            {totalCost !== null ? `$${totalCost.toLocaleString()}` : 'TBD'}
                          </p>
                        </div>
                      </div>
                    </div>
